...
(function (Kernel, Storage, Sandbox, Module, Utils) {
    ...
    App.prototype = {
        ...
        startAll: function (names) {
            var k;
            if (names && TOSTRING(names) === '[object Array]') {
                for (k = 0; hasOwn(registered, names[k]) && this.start(names[k]); k++) {}
            } else {
                for (k in registered) {
                    this.start(k);
                }
        ...
        stopAll: function (names) {
            var k;
            if (names && TOSTRING(names) === '[object Array]') {
                for (k = 0; hasOwn(registered, names[k]) && this.stop(names[k]); k++) {}
            } else {
                for (k in registered) {
                    this.stop(k);
                }
    ...
    Kernel.App = App;
})(Fea.Kernel, Fea.Kernel.Storage, Fea.Kernel.Sandbox, Fea.Kernel.Module, Fea.Kernel.Utils);
